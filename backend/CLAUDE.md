...
│   │           │   ├── project_lead_agent/ # Project Delivery OS runtime
...
│   │           ├── store/             # LangGraph PostgresStore provider + repositories
...
- The default built-in lead agent now acts as the user-facing coordinator for a hidden **project delivery team** on software tasks. It keeps the user interaction unified while routing internal work to specialist subagents and coordinating internal `ProjectBrief`, `WorkOrder`, `AgentReport`, and `GateDecision` contracts.

**Project Lead Agent** (`packages/harness/deerflow/agents/project_lead_agent/agent.py`):
- Entry point: `make_project_lead_agent(config: RunnableConfig)` registered in `langgraph.json`
- Compiled as a LangGraph phase router over `ProjectState`
- Routes each run to one project phase (`intake -> discovery -> architecture -> planning -> build -> qa_gate -> delivery -> closed`)
- Uses `ProjectLifecycleMiddleware` plus LangGraph Store projections (`projects/index`, `projects/snapshots`, `projects/controls`)
- Shares the `lead-agent` long-term memory namespace intentionally, without polluting the default ordinary-chat global memory
...
- DeerFlow now requires both `checkpointer` and `store` sections in `config.yaml`
- Both must use PostgreSQL (`type: postgres`)
- App startup, Gateway startup, and `make dev` all fail fast if either DSN is missing or unreachable
- SQLite and file-backed runtime persistence are no longer supported as primary paths
...
| **Memory** (`/api/memory`) | `GET /` - memory data; `POST /reload` - reload from LangGraph Store; `GET /config` - config; `GET /status` - config + data |
| **Projects** (`/api/projects`) | `POST /` - create project; `GET /` - list projects; `GET /{project_id}` - project board; `POST /{project_id}/actions` - pause/resume/abort |
| **Project Teams** (`/api/project-teams`) | `GET /` - list teams; `POST /{team_name}` - create; `GET /{team_name}` - read; `PUT /{team_name}` - update; `DELETE /{team_name}` - delete |
...
**Built-in Agents**:
- `general-purpose` (fallback cross-cutting worker)
...
- `ProjectBrief`: scoped objective, users, deliverables, constraints, success criteria, and project tags
- `WorkOrder`: owner, write/read scope, dependencies, verification, and done definition
- `AgentReport`: summary, findings/changes, risks, verification, blockers, and handoff
- `GateDecision`: `pass | pass_with_risk | fail` plus blocking issues and required rework

The lead-agent prompt uses these contracts to route software work through research → architecture/planning → implementation → QA gate → delivery packaging while keeping the end user talking to only one visible agent.
...
**Data Structure** (stored in LangGraph PostgresStore; legacy `memory.json` is import-only):
...
- `storage_path` - Legacy JSON import path used for migration/bootstrap only
...
- Supports project-specific helpers: `create_project()`, `list_projects()`, `get_project()`, `control_project()`, `project_chat()`
- Uses the configured PostgreSQL LangGraph checkpointer/store by default for persistence
