...
**Project Team Runtime** (`packages/harness/deerflow/project_runtime/graph.py`):
- Secondary entry point: `make_project_team_agent()` registered as `project_team_agent` in `langgraph.json`
- Uses explicit persisted phases instead of prompt-routed orchestration
- Stores project state only in `ProjectThreadState` via the LangGraph checkpointer
- Runs hybrid QA: executable acceptance checks are executed, non-executable checks are preserved as structured findings
- Writes a canonical `delivery_summary` before transitioning to `done`
...
- `runtime_name` - Optional explicit graph target for embedded client calls; defaults to `lead_agent`
...
`ProjectRuntimeConfig` adds `acp_allowed_specialists` and `default_model_name` for the optional `project_team_agent` path.
...
- `stream(message, thread_id)` — yields `StreamEvent` aligned with LangGraph SSE protocol:
  ...
  - `"end"` — stream finished
- `project_chat(message, thread_id)` — same contract, but targets `project_team_agent`
- `project_stream(message, thread_id)` — same contract, but targets `project_team_agent`
- Agent created lazily via `create_agent()` + `_build_middlewares()`, same as `make_lead_agent`
- Project runtime graphs are also created lazily and cached behind the same client instance
